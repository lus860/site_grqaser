...
@section('content')
    <h2>{{ __('category.add_new_category') }}</h2>
    <form method="post" action="{{ route('store_category') }}" id="form1"">
        @csrf
        <div class="row">
                    ...
                    <ul class="nav nav-tabs">
                        @foreach ($languages as $language)
                            <li class="@if($language->iso == 'hy') active @endif"><a
                                    href="#{{$language->iso}}-category-name" data-toggle="tab"
                    ...
                    <div class="tab-content">
                        @foreach ($languages as $language)
                            <div class="tab-pane @if($language->iso == 'hy') active @endif"
                                 id="{{$language->iso}}-category-name">
                                <div class="form-group">
                                    <input type="text" name="{{$language->iso}}-category-name" class="form-control"
                                           value="{{ old($language->iso.'-category-name') }}"
                                           placeholder="{{ __('category.name') }}"
                                           @error($language->iso.'-title') style="border:2px solid red" @enderror >
                                    @error($language->iso.'-category-name')
                            ...
                            <div class="tab-pane @if($language->iso == 'hy') active @endif"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="{{$language->iso}}-seo-title" class="form-control"
                                           ...
                                           placeholder="{{ __('category.seo_title') }}">
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="{{$language->iso}}-seo-key" class="form-control"
                                           ...
                                           placeholder="{{ __('category.seo_key') }}">
                                </div>
                                <div class="form-group">
                                    <label for="">{{ __('category.seo_description') }}</label>
                                    <textarea class="text" name="{{$language->iso}}-seo-description"
                                              placeholder="{{ __('category.seo_description') }}"
                                              style="width: 100%;min-height:175px;font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;resize: vertical!important;">{{ old($language->iso.'-seo-description') }}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
